...
def run_vdv_analysis():
    ...
    st.write(f'Total Annual Expenses for New Clusters: ₹{format_currency(total_expenses_new_district):,.2f}')
    st.write(f'Total Annual Expenses for Old Clusters (Adjusted VDVs): ₹{format_currency(total_expenses_old_district):,.2f}')
